...
; -------- initialize arrays for output
  ...
  if n_np gt 0 then begin 
     ...
     q = long((sqrt(0.25d + 0.5d*ipix[w_np])-0.5d))
     theta[w_np] = thetaq[q]
     phi[w_np] = (ipix[w_np]-ring0[q]+0.5d)/(nq/(2*!dpi))[q]
  endif 

  w_eq = where((np eq 0) and (ipix LT (npix-bound)), n_eq)
  if n_eq gt 0 then begin 
     qind = lindgen(2*nside-1)+nside
     thetaq[qind] = acos((2-(qind+1)/double(nside))*2.d/3.d)

     q = (ipix[w_eq]-bound)/(4*nside)+nside
     theta[w_eq] = thetaq[q]

     phi[w_eq] = (ipix[w_eq]-ring0[q]+(q AND 1)*0.5d)/(4*nside/(2*!dpi))[q]
  endif 

  w_sp = where(ipix GE (npix-bound), n_sp)
  if n_sp gt 0 then begin 
     qind = lindgen(nside)+(3*nside-1)
     thetaq[qind] =acos(-1+((qind+1-4*nside)/double(nside))^2/3.d)

     q = (4*nside-2)-long(sqrt(0.5d*((0.5d + npix-1)-ipix[w_sp]))-0.5d)
     theta[w_sp] = thetaq[q]
     phi[w_sp] = (ipix[w_sp]-ring0[q]+0.5d)/(nq/(2*!dpi))[q]
  endif 
