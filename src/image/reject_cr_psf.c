...
int reject_cr_psf(float *image, 
                  ...
                  short *rejected)
{
  float sigmaback[4],back[4],im,invsigma;
  ...
  for(i=1;i<xnpix-1;i++) {
    for(j=1;j<ynpix-1;j++) {
      ...
      if(image_ivar[j*xnpix+i]>0.) {
        ...
        if(invsigma*im<back[0]*invsigma+nsig) continue;
        back[1]=0.5*(image[(j-1)*xnpix+i]+image[(j+1)*xnpix+i]);
        if(invsigma*im<back[1]*invsigma+nsig) continue;
        back[2]=0.5*(image[(j-1)*xnpix+(i-1)]+image[(j+1)*xnpix+(i+1)]);
        if(invsigma*im<back[2]*invsigma+nsig) continue;
        back[3]=0.5*(image[(j+1)*xnpix+(i-1)]+image[(j-1)*xnpix+(i+1)]);
        if(invsigma*im<back[3]*invsigma+nsig) continue;
        ...
        if((invsigma*im-cfudge)*c2fudge*psfvals[0]>
           invsigma*(back[0]+cfudge*sigmaback[0])) {
        ...
        if((invsigma*im-cfudge)*c2fudge*psfvals[0]>
           invsigma*(back[1]+cfudge*sigmaback[1])) {
          rejected[j*xnpix+i]=lastreject;
          lastreject=j*xnpix+i;
        ...
        if((invsigma*im-cfudge)*c2fudge*psfvals[1]>
           invsigma*(back[2]+cfudge*sigmaback[2])) {
          rejected[j*xnpix+i]=lastreject;
          lastreject=j*xnpix+i;
        ...
        if((invsigma*im-cfudge)*c2fudge*psfvals[1]>
           invsigma*(back[3]+cfudge*sigmaback[3])) {
          rejected[j*xnpix+i]=lastreject;
          lastreject=j*xnpix+i;
