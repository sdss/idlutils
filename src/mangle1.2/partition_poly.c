...
/*------------------------------------------------------------------------------
  ...
  Return value: npoly = number of connected polygons in polys,
			...
			to lassoo a polygon.
*/
int partition_poly(poly, polys, npolys, all_oneborder, adjust_lassoo, overwrite_original, npoly_try)
...
#define DNP		4
    ...
    int i, ier, iev, in, ip, it, iv, ivm, ivthat, ivthis, nev, nev0, np, npoly, nv, nvm;
    int *ipv, *ev;
    double *ve, *vm, *angle;
    double *vmax, *vmin, *cmmin, *cmmax;
    double cmthat, cmthis, cth, s, sth, th, thm, ththat, ththis, tol, v[3], x[3], y[3];

    *npoly_try = 0;

    /* vertices and centres of edges of polygon */
    tol = mtol;
    ier = gverts(*poly, do_vcirc, &tol, &nv, nve, &ve, &angle, &ipv, &nev, &nev0, &ev);
    if (ier) return(-1);
    ...
    *npoly_try = nvm;
	...
	do {
	    ...
	    tol = mtol;
	    ier = gvlims(*poly, do_vcirc, &vm[3 * ivm], &tol, &nv, &vmin, &vmax, &cmmin, &cmmax, &nev, &nev0, &ev);
	    if (ier == -1) return(-1);
	    if (ier) break;
		...
		for (i = 0; i < 3; i++) {
		    v[i] = vm[i + 3 * ivm] + ((cmthis - cmthat) / s + 0.02 * (it + 1)) * v[i];
		}
