...
/* local functions */
char *get_word(), *get_keyword();
int rdmask(), new_fmt(), get_n(), get_nang();
polygon *get_poly(), *rd_poly(), *rd_circ(), *rd_edge(), *rd_rect();
...
extern polygon *vert_to_poly(), *edge_to_poly();
extern vertices *new_vert();
extern void msg(char *, ...);
extern void copy_format();
extern void free_poly(), free_vert();
extern void scale(), scale_azel();
extern void gc_v(), circ_to_rpcm(), az_to_rpcm(), el_to_rpcm();
...
*/ 
int rdmask(name, fmt, polys, npolys)
		 char *name;
		 format *fmt;
		 int npolys;
#ifdef GCC
		 ...
		 char *str, **str_rest;
{
	...
	/* first word in str */
	word = get_word(str, blank, 0, &wd_len);
	if (!word) return(0x0);
...
/*------------------------------------------------------------------------------
  ...
  Return value: 0 = ok;
	-1 = error.
*/
int new_fmt(keyword, line_rest, fmt)
		 char *keyword, **line_rest;
		 format *fmt;
{
	/* static char *Region_fmt = "%d ( %d caps, %d holes):"; */
	/* static char *generic_fmt = "%d ( %d caps, %lf weight ...:"; */
	static char *Region_fmt = "%d%*[^0-9]%d%*[^0-9]%d";
	static char *generic_fmt = "%d%*[^0-9]%d%*[^0-9-]%lf";
	static char *edges_fmt = "%d%*[^0-9]%d%*[^0-9]%d%*[^0-9-]%lf";
	static char *skip_fmt = "%d";
	static char *end_fmt = "%d";
	static char *unit_fmt = " %c";
	int ird, nholes;

	/* Regions */
	if (strcmp(keyword, "Region") == 0) {
		fmt->in = keyword;
		ird = sscanf(*line_rest, Region_fmt, &fmt->id, &fmt->n, &nholes);
		if (ird != 3) {
	    WHERE;
    ...
    /* polygons, circles, rectangles, vertices */
	    ...
	    ird = sscanf(*line_rest, edges_fmt, &fmt->id, &fmt->innve, &fmt->n, &fmt->weight);
	    /* default number of edges per line is variable (fmt->n = 0) */
	    if (ird < 3) fmt->n = 0;
	    /* default number of points/edge is 2 */
	    if (ird < 2) fmt->innve = 2;
		} else {
	    ird = sscanf(*line_rest, generic_fmt, &fmt->id, &fmt->n, &fmt->weight);
	    if (ird < 2) {
				/* polygon format requires at least 2 integer arguments */
				if (strcmp(keyword, "polygon") == 0) {
					WHERE;
					fprintf(stderr, " expecting two integers <id> <number_of_caps> following keyword polygon\n");
					return(-1);
		 ...
		 format *fmt;
{
	...
	for (ip = 0; ip < fmt->n; ip++) {
		...
		ird = sscanf(file.line, "%lf %lf %lf %lf",
								 &poly->rp_(0, ip), &poly->rp_(1, ip), &poly->rp_(2, ip), &poly->cm[ip]);
		if (ird != 4) {
	    WHERE;
		 ...
		 format *fmt;
{
	...
	for (icirc = 0; icirc < ncirc; icirc++) {
		...
		circ_to_rpcm(angle, &poly->rp_(0, icirc), &poly->cm[icirc]);
	}
...
#define DNV		4
	...
	char unit;
	char *next, *word;
	int i, iang, iedge, iev, ird, iv, ive, ivert, nedge, nv, nve, nvert, reverse;
	double *angle;
	double az, el;
	polygon *poly = 0x0;
	...
	for (iedge = 0; iedge < nedge; iedge++) {
	    ...
	    /* phase azimuth to first angle */
	    if (ivert > 0) az -= rint((az - vert->v[0].az) / TWOPI) * TWOPI;
...
#define ROUND		1.e-5
	...
	for (irect = 0; irect < nrect; irect++) {
	    ...
	    if (fabs(daz - PI) <= ROUND) {
				i = 0;
				az_to_rpcm(angle[i], i % 2, &poly->rp_(0, ip), &poly->cm[ip]);
				ip++;
				/* need two caps */
	    } else {
	    ...
	    if ((i == 2)? (angle[i] > - PIBYTWO) : (angle[i] < PIBYTWO)) {
				el_to_rpcm(angle[i], i % 2, &poly->rp_(0, ip), &poly->cm[ip]);
				ip++;
	    }
		}
	}
